...
######  ONLY ADJUSTED FOR NO RANDOM EFFECTS PART
est.glmmElastic<-function(fix,rnd,data,lambda,alpha,family=gaussian(link = "identity"),
...
{  
  ...
  if(!is.null(rnd))
  {
    return.obj <- est.glmmElastic.RE(fix=fix,rnd=rnd,data=data,lambda=lambda,alpha=alpha,family=family,
                                   final.re=final.re,switch.NR=switch.NR,control=control)
  }else{
     return.obj <- est.glmmElastic.noRE(fix=fix,data=data,lambda=lambda,alpha=alpha,family=family,
                                      final.re=final.re,switch.NR=switch.NR,control=control)
  }
  return(return.obj)
}
...
glmmElastic <- function(fix=formula, rnd=formula, data, lambda, alpha, family=gaussian(link = "identity"), switch.NR = FALSE, final.re=FALSE, 
                      control=list()){
  est <- est.glmmElastic(fix,rnd,data=data,lambda=lambda,alpha=alpha,family=family,
                       switch.NR=switch.NR,final.re=final.re,control=control)
  
  est$fitted.values <- est$y_hat
  est$StdDev <- est$Q
  est$call <- match.call()
  class(est) <- "glmmElastic"
  est
}


print.glmmElastic <- function(x, ...)
{
  cat("Call:\n")
  print(x$call)
  cat("\nFixed Effects:\n")
...
summary.glmmElastic <- function(object, ...)
{
  se <- object$fixerror
...
predict.glmmElastic <- function(object,newdata=NULL,new.random.design=NULL, smooth=NULL,...)
{
  ...
  B <- model.matrix(smooth$formula, newdata)
      ...
      {   
        if(rnd.len==1)
        {
          ...
          if(krit.random)
          {
            ...
            if(nrow(X)!=1)
            {
              tmp <- cbind(X[,is.element(colnames(X),names(object$coef))],W.neu)[rand.ok,]%*%c(object$coef[is.element(names(object$coef),colnames(X))],object$ranef[match(colnames(W.neu),names(object$ranef))])
              y[rand.ok]<- family$linkinv(tmp+ Phi[rand.ok,]%*% object$smooth) 
            }else
            {
              y[rand.ok]<- family$linkinv(c(X[,is.element(colnames(X),names(object$coef))],W.neu)[rand.ok]%*%c(object$coef[is.element(names(object$coef),colnames(X))],object$ranef[match(names(W.neu),names(object$ranef))]))}
          }else{
