est.glmmElastic.RE<-function(fix,rnd,data,lambda,alpha,family,final.re,switch.NR,control)
{  
 ...
 #==================================================================== 
  ...
  if(switch.NR)
  {
    ...
    {  
      ...
      lambda.max<-max(abs(score_vec[(q+1):lin]))/alpha
      
      if (BLOCK)
      {
# CHANGE  *********        
#        grad.1<-gradient.lasso.block(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda,block=block)
        grad.1<-gradient.elastic.block(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda,alpha.b=alpha,block=block)
      }else{
# CHANGE
#        grad.1<-gradient.lasso(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda)
      ...
      lambda.max<-max(abs(score_vec2[(q+1):lin]))/alpha
      
      if (BLOCK)
      { #CHANGE
        grad.1<-gradient.elastic.block(score.beta=score_vec2[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda,alpha.b=alpha,block=block)
      }else{
#CHANGE        grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda)
        grad.1<-gradient.elastic(score.beta=score_vec2[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda, alpha.b=alpha)
      }
      score_vec2[(q+1):lin] <- grad.1
...
#CHANGE          
          ...
          {
#CHANGE
            grad.1<-gradient.elastic.block(score.beta=score_vec2[(q+1):lin],b=Delta[l,(q+1):lin],lambda.b=lambda,alpha.b=alpha,block=block)
          }else{
#CHANGE            
#            grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[l,(q+1):lin],lambda.b=lambda)
            grad.1<-gradient.elastic(score.beta=score_vec2[(q+1):lin],b=Delta[l,(q+1):lin],lambda.b=lambda, alpha.b=alpha)
          }
          score_vec2[(q+1):lin] <- grad.1
