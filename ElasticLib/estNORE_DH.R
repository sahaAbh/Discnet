est.glmmElastic.noRE<-function(fix,data,lambda,alpha,family,final.re,switch.NR,control)
{  
# CHANGE: We changed the name and testing with trial inputs:  fix = formula.1; switch.NR=F;  alpha=0.5;
  if(grepl("\\*", fix[3]))
  ...
  if(switch.NR)
  {
    ...
    {  
      ...
      if(
        is.null(family$multivariate)){
        score_vec<-t(Z_alles)%*%((y-Mu)*D*SigmaInv)
      }else{
        score_vec<-t(Z_alles)%*%(D%*%(SigmaInv%*%(y-Mu)))
      }
#CHANGE      lambda.max<-max(abs(score_vec[(q+1):lin]))
      
      if (BLOCK)
      {
        grad.1<-gradient.lasso.block(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda,block=block)
      }else{
# CHANGE        grad.1<-gradient.lasso(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda)
        grad.1<-gradient.elastic(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda, alpha.b=alpha)
      }
...
# CHANGE  
      if(is.null(family$multivariate)){
        score.unpen.vec <-t(Z_alles)%*%((y-Mu)*D*SigmaInv);
        lambda.max<-max(abs(score.unpen.vec[(q+1):lin]))/alpha}
...
# CHANGE         lambda.max<-max(abs(score_vec2[(q+1):lin]))/alpha
          
          if (BLOCK)
          {
            grad.1<-gradient.lasso.block(score.beta=score_vec2[(q+1):lin],b=Delta[l-1,(q+1):lin],lambda.b=lambda,block=block)
          }else{
# Change        #    grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[l-1,(q+1):lin],lambda.b=lambda)
            grad.1<-gradient.elastic(score.beta=score_vec2[(q+1):lin],b=Delta[l-1,(q+1):lin],lambda.b=lambda, alpha.b=alpha)   
          }
...
# CHANGE: ADDITION OF K TERM
            K_act = lambda* (1-alpha)*rep(1,lin_akt)
            K_act[1:q]= 0;
                     if(length(K_act)!= 1)
                     K_act= diag(K_act);            
            if(is.null(family$multivariate)){
              D <- as.vector(D);SigmaInv <- as.vector(SigmaInv)
              F_gross<-(t(Z_aktuell)%*%(Z_aktuell*D*SigmaInv*D)+K_act)
            }else{
...
# CHANGE  
      ...
      if(!final.re)
      {
        ...
        if(is.null(family$multivariate)){
          D <- as.vector(D);SigmaInv <- as.vector(SigmaInv)
          W_opt <- D*SigmaInv*D
          F_gross <- t(Z_aktuell)%*%(Z_aktuell*W_opt) + K_act
        }else{
...
# CHANGE          lambda.max<-max(abs(score_vec2[(q+1):lin]))
...
#            grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[l-1,(q+1):lin],lambda.b=lambda)
          ...
          {
            lin_akt<-q+sum(!is.element(Delta[l-1,(q+1):lin],0))
 #CHANGE : Modifying PKT to incorporate the ridge of beta  
            K_act = lambda* (1-alpha)*rep(1,lin_akt)
            K_act[1:q]= 0;
  #         if(length(K_act)!= 1)
  #         K_act= diag(K_act);            
            P_akt<-c(K_act,penal.vec)
            if(is.null(family$multivariate)){
              D <- as.vector(D);SigmaInv <- as.vector(SigmaInv)
