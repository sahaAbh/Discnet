...
est.glmmElastic_int<-function(fix,rnd,data,p1,lambda1,lambda2,alpha,family=gaussian(link = "identity"),
...
{  
  ...
  if(!is.null(rnd))
  {
    return.obj <- est.glmmElastic_int.RE(fix=fix,rnd=rnd,data=data,p1=p1,lambda1=lambda1,lambda2=lambda2,alpha=alpha,family=family,
...
glmmElastic_int <- function(fix=formula, rnd=formula, data, p1,lambda1,lambda2, alpha, family=gaussian(link = "identity"), switch.NR = FALSE, final.re=FALSE, 
                      control=list()){
  est <- est.glmmElastic_int(fix,rnd,data=data, p1=p1,lambda1=lambda1,lambda2=lambda2,alpha=alpha,family=family,
