est.glmmElastic_int.RE<-function(fix,rnd,data,p1,lambda1,lambda2,alpha,family,final.re,switch.NR,control)
{  
  
  control<-do.call(glmmElasticControl, control)
...
#CHANGE    very.old.names<-very.old.names[!is.na(control$index)]
    very.old.names<-orig.names[!is.na(control$index)]
    ############
    ############
    ############
  }else
 ...
 #==================================================================== 
  ...
  #CHANGE: addition of gamma for interaction
  tmp<-t(combn(1:p1,2)); tmp1 <-paste0("V",tmp[,1],":V",tmp[,2],":")
  p_int <- p1*(p1-1)/2
  
  if(is.null(control$int_start))
    control$int_start<-rep(0,p_int)
  ...
  gamma_int_null <- control$int_start
...
#CHNAGE: Reassigning the Delta abnd gamma's          
          Delta[1,]<-Delta_start+min(t_opt,t_edge1)*nue*(0.5^half.index)*score_vec
          Delta_gamma[1,]<-Delta_gamma[1,]+min(t_opt,t_edge2)*nue*(0.5^half.index)*score_vec[(lin-p_int+1):lin]
          Delta[1,(lin-p_int+1):lin] <-  
    ...
    {  
      ...
      Z_alles<-cbind(X,U,Phi,W)
      XX  <- Z_alles[,(lin-p_int+1):lin]
  #CHANGE ADDED: Delta_gamma: the code needs modifications as the order of beta changes to 
  # to separate unpenalized ones    
      ########################################################## some definitions ################################################
      Delta_gamma<-matrix(0,control$steps,p_int)
      Delta_gamma[1,] <- gamma_int_null
      Delta<-matrix(0,control$steps,(lin+dim.smooth+n%*%s))
      Delta[1,1:lin]<-beta_null[final.names]
      Delta[1,(lin+1):(lin+dim.smooth)]<-smooth_null
      Delta[1,(lin+dim.smooth+1):(lin+dim.smooth+n%*%s)]<-t(ranef_null)
...
# CHANGE IS REQUIRED FOR S(THETA) FUNCTION     
      if(is.null(family$multivariate)){
        score_vec<-t(Z_alles)%*%((y-Mu)*D*SigmaInv)-P1%*%Delta[1,]
        C1_mat <- C_fun(tmp1,beta_null[2:(p1+1)],Delta_gamma[1,])
        score_vec[2:(1+p1)] <-  score_vec[2:(p1+1)]+ (C1_mat%*% t(XX))%*%((y-Mu)*D*SigmaInv)
        C2_mat <- C_fun(tmp1,beta_null[2:(p1+1)],Delta_gamma[1,],type=2)
        score_vec[(lin-p_int+1):lin] <-  score_vec[(lin-p_int+1):lin] *C2_mat      }else
        {
        score_vec<-t(Z_alles)%*%(D%*%(SigmaInv%*%(y-Mu)))-P1%*%Delta[1,]
      }  
      lambda1.max<-max(abs(score_vec[(q+1):(lin-p_int)]))/alpha
      lambda2.max<-max(abs(score_vec[(lin-p_int+1):lin]))/alpha
...
# CHANGE
#        grad.1<-gradient.lasso(score.beta=score_vec[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda)
        grad.1<-gradient.elastic(score.beta=score_vec[(q+1):(lin-p_int)],b=Delta[1,(q+1):(lin-p_int)],lambda.b=lambda1, alpha.b=alpha)
        grad.2<-gradient.elastic(score.beta=score_vec[(lin-p_int+1):lin],b=Delta_gamma[1,],lambda.b=lambda2, alpha.b=alpha)
         }
      
      score_vec[(q+1):(lin-p_int)] <- grad.1
      score_vec[(lin-p_int+1):lin] <- grad.2
      
      Theta <-Delta[1,]
      Theta[(lin-p_int+1):lin] <- Delta_gamma[1,]
      crit.obj<-t.change(grad=score_vec[(q+1):lin],b= Theta[(q+1):lin])
      t_edge<-crit.obj$min.rate
      ...
      {  
        ...
        # CHANGE IS REQUIRED:
        Theta <- Theta + min(t_opt,t_edge)*nue*(0.5^half.index)*score_vec
        if(t_opt>t_edge & half.index==0)
          Theta[crit.obj$whichmin+q]<-0
        
        Delta_gamma[1,] <- Theta[(lin-p_int+1):lin]
        tmp2<- combn(Theta[2:(p1+1)],2);
        Delta[1,] <- Theta
        Delta[1,(lin-p_int+1):lin] <- (tmp2[1,]*tmp2[2,])*Delta_gamma[1,]
        Eta<-Z_alles%*%Delta[1,]
...
#CHANGE REQUIRED FOR THE 2ND TIME ON SCORE-VEC ITER 1      
      if(is.null(family$multivariate)){
        score_vec2<-t(Z_alles)%*%((y-Mu)*D*SigmaInv)-P1%*%Delta[1,]
        C1_mat <- C_fun(tmp1,Delta[1,2:(p1+1)],Delta_gamma[1,])
        score_vec2[2:(1+p1)] <-  score_vec2[2:(p1+1)]+ (C1_mat%*% t(XX))%*%((y-Mu)*D*SigmaInv)
        C2_mat <- C_fun(tmp1,Delta[1,2:(p1+1)],Delta_gamma[1,],type=2)
        score_vec2[(lin-p_int+1):lin] <-  score_vec2[(lin-p_int+1):lin] *C2_mat   
      }else{
        score_vec2<-t(Z_alles)%*%(D%*%(SigmaInv%*%(y-Mu)))-P1%*%Delta[1,]
      }  
      
      lambda1.max<-max(abs(score_vec2[(q+1):(lin-p_int)]))/alpha
      lambda2.max<-max(abs(score_vec2[(lin-p_int+1):lin]))/alpha
      
      if (BLOCK)
      {
        grad.1<-gradient.lasso.block(score.beta=score_vec2[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda,block=block)
      }else{
#CHANGE        grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[1,(q+1):lin],lambda.b=lambda)
        grad.1<-gradient.elastic(score.beta=score_vec2[(q+1):(lin-p_int)],b=Delta[1,(q+1):(lin-p_int)],lambda.b=lambda1, alpha.b=alpha)
        grad.2<-gradient.elastic(score.beta=score_vec2[(lin-p_int+1):lin],b=Delta_gamma[1,],lambda.b=lambda2, alpha.b=alpha)
      }
      
      score_vec2[(q+1):(lin-p_int)] <- grad.1
      score_vec2[(lin-p_int+1):lin] <- grad.2
      
      Theta <-Delta[1,]
      Theta[(lin-p_int+1):lin] <- Delta_gamma[1,]
      crit.obj<-t.change(grad=score_vec2[(q+1):lin],b= Theta[(q+1):lin])
      t_edge<-crit.obj$min.rate
...
#CHANGE IS REQUIRED HERE: Theta is already updated 
            Theta <- Theta + min(t_opt,t_edge)*nue*(0.5^half.index)*score_vec
            if(t_opt>t_edge & half.index==0)
              Theta[crit.obj$whichmin+q]<-0
            
            Delta_gamma[l,] <- Theta[(lin-p_int+1):lin]
            tmp2<- combn(Theta[2:(p1+1)],2);
            Delta[l,] <- Theta
            Delta[l,(lin-p_int+1):lin] <- (tmp2[1,]*tmp2[2,])*Delta_gamma[l,]
  ...
  # CHANGES        
          if(is.null(family$multivariate)){
            ...
            C1_mat <- C_fun(tmp1,Delta[l,2:(p1+1)],Delta_gamma[l,])
            score_vec2[2:(1+p1)] <-  score_vec2[2:(p1+1)]+ (C1_mat%*% t(XX))%*%((y-Mu)*D*SigmaInv)
            C2_mat <- C_fun(tmp1,Delta[l,2:(p1+1)],Delta_gamma[l,],type=2)
            score_vec2[(lin-p_int+1):lin] <-  score_vec2[(lin-p_int+1):lin] *C2_mat   
          }else
            {
...
#CHANGE            
#            grad.1<-gradient.lasso(score.beta=score_vec2[(q+1):lin],b=Delta[l,(q+1):lin],lambda.b=lambda)
            grad.1<-gradient.elastic(score.beta=score_vec2[(q+1):(lin-p_int)],b=Delta[l,(q+1):(lin-p_int)],lambda.b=lambda1, alpha.b=alpha)
            grad.2<-gradient.elastic(score.beta=score_vec2[(lin-p_int+1):lin],b=Delta_gamma[l,],lambda.b=lambda2, alpha.b=alpha)
          }
          ...
          Theta <-Delta[l,]
          Theta[(lin-p_int+1):lin] <- Delta_gamma[l,]
          crit.obj<-t.change(grad=score_vec2[(q+1):lin],b= Theta[(q+1):lin])
          t_edge<-crit.obj$min.rate
      ...
      ret.obj$aic<-aic
      ret.obj$bic<-bic
      ret.obj$Delta_gamma<-Delta_gamma[conv.step,]
      ret.obj$Deltamatrix<-Delta
      ret.obj$smooth<-Delta_neu[(lin+1):(lin+dim.smooth)]
      ret.obj$ranef<-Delta_neu[(lin+dim.smooth+1):(lin+dim.smooth+n%*%s)]
      ret.obj$coefficients<-Delta_neu[1:(lin)]
      ...
      ret.obj$df<-df
      ret.obj$loglik<-loglik
      ret.obj$lambda1.max<-lambda1.max
      ret.obj$lambda2.max<-lambda2.max
      ret.obj$logLik.vec<-logLik.vec
      ret.obj$rnd <- rnd
      return(ret.obj)
    }  
